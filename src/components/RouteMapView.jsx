...
const buildStopIcon = (color, number) => {
  ...
  const icon = L.divIcon({
    html: `<div style="
      ...
      background:${color}; border:2px solid #ffffff; box-shadow:0 2px 5px rgba(7,11,20,0.4);
      display:flex; align-items:center; justify-content:center;
    "><span style="transform: rotate(45deg); color:#fff; font-size:11px; font-weight:700;">${number}</span></div>`,
        ...
        <MapContainer center={allPositions[0]} zoom={12} style={{ height: '420px', width: '100%' }} scrollWheelZoom={true}>
          ...
          {dayLines.map(d => (
            <Polyline
              ...
              pathOptions={{ color: d.color, weight: 4, opacity: 0.85, lineCap: 'round', lineJoin: 'round' }}
            />
          ))}

          {dayLines.map(d => d.stops.map((s, i) => (
            <Marker key={`${d.day}-${s.place.id}`} position={s.coords} icon={buildStopIcon(d.color, i + 1)}>
              <Tooltip direction="top" offset={[0, -20]}>Dia {d.day} · {s.place.name}</Tooltip>
            </Marker>
          )))}
        </MapContainer>
      </div>
